...
USE [BDAdvocacia] -- Specify the database in which the objects will be created.

SET NOCOUNT ON

DECLARE @CreateJobs nvarchar(max)          = 'N'         -- Specify whether jobs should be created.
DECLARE @BackupDirectory nvarchar(max)     = N'C:\Backup'        -- Specify the backup root directory. If no directory is specified, the default backup directory is used.
